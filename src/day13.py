...
def find_reflection_smudge(pattern):
    ...
    for row_idx in range(rows):
        ...
        if different_values == 1:
            different_idx = row_idx
            print("different row: values: ", different_values)
        # if all_columns and (up_idx == 0 and down_idx == rows) or (up_idx == -1 and down_idx == rows - 1):
        if all_columns:
            # print("FOUND A BREAKING POINT: ", row_idx, up_idx, down_idx, rows)
            row_sum += row_idx * 100
    ...
    for col_idx in range(cols):
        ...
        if different_values == 1:
            different_idx = col_idx
            print("different column values: ", different_values)
        # if all_rows and (left_idx == 0 and right_idx == cols) or (left_idx == -1 and right_idx == cols - 1):
        if all_rows:
            # print("COLUMN BREAKING POINT: ", col_idx, left_idx, right_idx, cols)
            col_sum += col_idx
...
for idx, pattern in enumerate(patterns):
    # print(idx)
    ans += find_reflection_smudge(pattern)
